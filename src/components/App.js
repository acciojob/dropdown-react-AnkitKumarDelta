...
function App() {
	...
	function handleStateChange(e) {
		const stateInd = parseInt(e.target.value, 10);
		setSelectedState(stateInd);
		setSelectedCity(0);
	...
	function handleLandmarkChange(e) {
		const landmarkIndex = parseInt(e.target.value, 10);
	...
	return (
		<div id="main">
			...
			<select id="city" onChange={handleCityChange} value={selectedCity}>
				{states[selectedState].city.map((city, ind) => (
					<option key={ind} value={ind}>{city.name}</option>
				))}
			</select>

			<select id="landmark" onChange={handleLandmarkChange} value={selectedLandmark}>
				{states[selectedState].city[selectedCity].landmarks.map((landmark, ind) => (
					<option key={ind} value={ind}>{landmark.name}</option>
				))}
			</select>
			...
			<div id="landmark-name">{states[selectedState].city[selectedCity].landmarks[selectedLandmark].name}</div>
